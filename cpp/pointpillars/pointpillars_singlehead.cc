...
#include "pointpillars_singlehead.h"

#include <chrono>
#include <iostream>
#include <iostream>

PointPillarsSingle::PointPillarsSingle(const float score_threshold,
...
PointPillarsSingle::~PointPillarsSingle() {
    // for pillars 
...
void PointPillarsSingle::InitParams()
{
    ...
    kNumPointFeature = 4; //4
    // the input feature number to the pfe
    kNumGatherPointFeature = 10;
    kNumInputBoxFeature = 7;
    ////TODO: delete below line
    kNumOutputBoxFeature = 9;
    ...
    kGridXSize = static_cast<int>((kMaxXRange - kMinXRange) / kPillarXSize); //432
    kGridYSize = static_cast<int>((kMaxYRange - kMinYRange) / kPillarYSize); //496
    kGridZSize = static_cast<int>((kMaxZRange - kMinZRange) / kPillarZSize); //1
    // std::cout<<"RPN input size: 64 * "<<kGridXSize<<" * "<<kGridYSize <<std::endl;
...
void PointPillarsSingle::DeviceMemoryMalloc() {
    // for pillars 
    ...
    GPU_CHECK(
        cudaMalloc(&pfe_buffers_[0], kMaxNumPillars * kMaxNumPointsPerPillar *
                                        kNumGatherPointFeature * sizeof(float))); //the input to the pfe should be 40000*32*10
...
void PointPillarsSingle::SetDeviceMemoryToZero() {
...
void PointPillarsSingle::InitTRT(const bool use_onnx) {
    if (use_onnx_) {
    ...
    if (pfe_context_ == nullptr || backbone_context_ == nullptr) {
        ...
        std::cout << "Successfully init the tensorRT model!"<<std::endl;
    }
...
void PointPillarsSingle::DoInference(const float* in_points_array,
