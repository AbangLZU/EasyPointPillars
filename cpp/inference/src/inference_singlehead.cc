...
#include <string>
// headers in 3rd-part
#include "../pointpillars/pointpillars_singlehead.h"
...
// read kitti velodyne bin data for testing
int Bin2Array(float* &points_array , string file_name , int num_feature = 4){
  ifstream InFile;
  InFile.open(file_name.data());
  assert(InFile.is_open());
  ...
  printf("vector length: %d", temp_points.size());
  points_array = new float[temp_points.size()];
  for (int i = 0 ; i < temp_points.size() ; ++i) {
    points_array[i] = temp_points[i];
  }
